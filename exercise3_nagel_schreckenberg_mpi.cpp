...
void update_velocities(Grid& grid, size_t vmax, double prob, std::uniform_real_distribution<double>& dist,
                       std::mt19937& gen)
{
...
int main(int argc, char* argv[])
{
    ...
    if (size == 2) {
        if (rank == 1) {
            ...
            int received_avg;
            MPI_Recv(&received_avg, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            std::cout << "Process 0 received average: " << received_avg << " from process 1\n";
        }
    }
